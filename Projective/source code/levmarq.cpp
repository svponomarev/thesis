...
*/
void solve_axb_cholesky(int n, void *l, void *x, void *b)
{
  int i,j;
  double sum;

    double (*arr_l)[n] = static_cast<double (*)[n]>(l);
    double *arr_x = static_cast<double *>(x);
    double *arr_b = static_cast<double *>(b);
  /* solve L*y = b for y (where x[] is used to store y) */

  for (i=0; i<n; i++) {
    sum = 0;
    for (j=0; j<i; j++)
      sum += arr_l[i][j] * arr_x[j];
    arr_x[i] = (arr_b[i] - sum)/arr_l[i][i];
  }
  ...
  for (i=n-1; i>=0; i--) {
    sum = 0;
    for (j=i+1; j<n; j++)
      sum += arr_l[j][i] * arr_x[j];
    arr_x[i] = (arr_x[i] - sum)/arr_l[i][i];
  }
}
...
*/
int cholesky_decomp(int n, void *l, void *a)
{
  int i,j,k;
  double sum;

double (*arr_l)[n] = static_cast<double (*)[n]>(l);
double (*arr_a)[n] = static_cast<double (*)[n]>(a);

  for (i=0; i<n; i++) {
    for (j=0; j<i; j++) {
      sum = 0;
      for (k=0; k<j; k++)
	sum += arr_l[i][k] * arr_l[j][k];
      arr_l[i][j] = (arr_a[i][j] - sum)/arr_l[j][j];
    }

    sum = 0;
    for (k=0; k<i; k++)
      sum += arr_l[i][k] * arr_l[i][k];
    sum = arr_a[i][i] - sum;
